...
#### OFI subscripts
* $\text{OF}_{m,b}^{i,n}$ means the bid‐side OFI at level m for instrument i on event n.
...
To account for liquidity, each level-m OFI is normalized by the average book depth $Q^{M,h}_{i,t}$:
